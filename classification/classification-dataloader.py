...
# export HF_TOKEN=
# python classification-dataloader.py --model_name mental/mental-roberta-base --dataset_name ESConv
...
# Define the balancing functions for SAD and ESConv datasets
def balance_sad_dataset(df, target_samples_per_label=1500):
    """
    Balance the SAD dataset to have equal samples per label.
    For labels with fewer samples than target, generate new samples by randomly removing
    percentages of words from the beginning and end of existing samples.
    For labels with more samples than target, randomly select the target number.
    """
    balanced_samples = []
    ...
    for label, group in grouped:
        ...
        if original_count < target_samples_per_label:
            ...
            # Generate additional samples by removing words from beginning/end
            ...
            for _ in range(samples_to_generate):
                ...
                if len(words) <= 3:  # If text is too short, just duplicate it
                    balanced_samples.append(sample)
                    continue
                
                # Randomly decide how much to remove from beginning and end
                # Ensure we keep at least 70% of the text
                max_remove_percent = min(30, 100 * (len(words) - 3) / len(words))
                remove_percent = random.uniform(0, max_remove_percent)
                
                # Split the removal between beginning and end
                begin_remove_percent = random.uniform(0, remove_percent)
                end_remove_percent = remove_percent - begin_remove_percent
                
                # Calculate how many words to remove
                begin_remove = int(len(words) * begin_remove_percent / 100)
                end_remove = int(len(words) * end_remove_percent / 100)
                
                # Create new text with words removed
                new_words = words[begin_remove:len(words)-end_remove]
                sample['text'] = ' '.join(new_words)
...
def load_dataset():
    if args.dataset_name == "SAD":
        url = "https://github.com/PervasiveWellbeingTech/Stress-Annotated-Dataset-SAD/raw/main/SAD_v1.zip"
        response = requests.get(url)
        zip_file = zipfile.ZipFile(BytesIO(response.content))

        with zip_file.open('SAD_v1.xlsx') as f:
            df = pd.read_excel(f, engine='openpyxl')

        df = df[['sentence', 'original_label']].dropna()
        df = df.rename(columns={'sentence': 'text', 'original_label': 'label'})
        ...
        # Verify we have 9 classes
        assert len(le.classes_) == 9, "Should have 9 classes"
        
        # First split the dataset to ensure test and validation sets remain unbalanced
        ...
        if args.balanced:
            print(f"Balancing ONLY the training set of SAD dataset to have {args.samples_per_label} samples per label...")
            train_df = balance_sad_dataset(train_df, target_samples_per_label=args.samples_per_label)
        ...
        # Filter out labels with fewer than 15 samples for ESConv dataset
...
def save_dataset_splits(dataset_dict, save_dir):
    ...
    for split_name, split_dataset in dataset_dict.items():
        # Convert to pandas DataFrame - using np.asarray instead of np.array with copy=False
        data = {}
        for column in split_dataset.features:
            # Use list() to get the data without relying on NumPy's array conversion
            data[column] = list(split_dataset[column])
...
def save_split_indices(dataset_dict, save_dir):
    ...
    split_indices = {}
    for split_name, split_dataset in dataset_dict.items():
        split_indices[split_name] = split_dataset.indices
